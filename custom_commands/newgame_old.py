...
# Cache champion data
champion_data = {}
async def init_champ_data():
    global champion_data
    async with aiohttp.ClientSession() as session:
        async with session.get(CHAMPION_DATA_URL) as response:
            data = await response.json()
            champ_data = {str(v['key']): v['name'] for v in data['data'].values()}
    return champion_data.update(champ_data)
asyncio.run(init_champ_data())



def get_champion_name(champion_id):
    return champion_data.get(str(champion_id), f"Unknown Champion {champion_id}")
...
async def format_mastery_data(mastery_data):
    ...
    for champ in top_champs:
        name = get_champion_name(champ['championId'])
...
class Summoner:
    def __init__(self, participant):
        ...
        self.champion = get_champion_name(self.champion_id)
        self.runes = []
    ...
    def champion_emote(self):
        formatted_name = self.champion.replace(' ', '').replace('.', '')
        if formatted_name in MatchData.EMOTES:
...
class MatchData:
    ...
    def __init__(self, match_id):
        ...
        self._data = None
        self.listen_for_match_updates()

    def listen_for_match_updates(self):
        async def listen(match_id):
            for _ in range(30): # check 30 times
                await asyncio.sleep(60*5) # every 5 minutes
                await self.get_match_data()
                if self.completed():
                    return
        asyncio.create_task(listen(self.match_id))

    
    async def initialize(self):
        await self.get_match_data()
        if MatchData.EMOTES == {}:
            MatchData.EMOTES = {str(emoji.name) : emoji for emoji in await bot.fetch_application_emojis()}
    ...
    def summoners(self):
        ...
        summoners = { p['puuid'] : Summoner(p) for p in data['info']['participants']}
...
class Player:
    ...
    async def get_current_match_id(self):
        ...
        if os.environ.get('ENV', 'prod') == 'dev':
            return await self.get_most_recent_match_id()
        if not self.puuid:
            return None
...
class Match:
    ...
    def description(self):
        ...
        if len(self.player_preferences) >= 10:
            ...
            lane_matchups = ""
            summoners = self.match_data.summoners() if self.match_data and self.match_data.summoners() else {}
            ...
            for role in ROLE_EMOTES:
                ...
                lane_matchups += f"{red_champ} `{left}` {emoji} `{right}` {blue_champ} \t{red_mention} vs. {blue_mention}\n"
            ...
            for _ in range(60): # 60 times
                ...
                for player in self.player_preferences.values():
                    ...
                    if games[match_id] >= 5:
                        ...
                        return
                    if match_id is not None and os.environ.get('ENV', 'prod') == 'dev':
                        self.match_id = match_id
                        self.match_data = MatchData(match_id)
                        await self.match_data.initialize()
                        return
        asyncio.create_task(listen(self))
...
async def new_game(ctx):
    ...
    try:
        ...
        emotes = {str(e.name): e for e in await bot.fetch_application_emojis()}
